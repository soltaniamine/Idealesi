...
const BrainwritingAddLine = ({camera,id, insertBrainwriting, scale}) => {
    ...
    return (
        layer.type == 22 ? (
          <div
            ...
            style={{
              transform: `translate(
                calc(${ scale == 1 ? `${x + width/2}px - 50%` : `${ bounds.right+bounds.width/2}px - 50%`}),
                calc(${ scale == 1 ? `${y + height+20}px - 50%` : `${bounds.bottom+20}px - 50%`})
              )
                scale(${scale})
              `,
...
export default BrainwritingAddLine