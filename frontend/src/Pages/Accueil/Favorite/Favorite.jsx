  ...
  const handleProfileClick = () => {
    setShowProfile(prevState => !prevState);
        ...
        const fetchSearch = async () => {
            try {
                const response = await axios.post('http://127.0.0.1:5000/search_projects', {filters: {favori:true}, keyword: word, user_id: uid });
                console.log(word);
                        ...
                        {isClicked ? (
                            <div  className="z-50 w-[50%] ml-8 mt-1.5 border-2 border-gray-100 bg-gray-100 rounded-xl  text-gray-600 ">
                                <div className=" w-[100%] h-12 flex items-center border-b-2 border-gray-700 rounded-t-xl pl-3  cursor-pointer">
                                    ...
                                    <input className="focus:outline-none ml-2 mr-7 bg-slate-100 w-[80%]" type="text" placeholder="Search..." value={word} onChange={(e)=>setWord(e.target.value)}/> 
                                    <div>
                                        <svg onClick={() => {
                                                setIsClicked(false);
